...
async def calc_price_final(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"◾ Доставка (Китай → Владивосток): {delivery} ₽\n\n"
            f"📦  <b>Дополнительно</b>: после прибытия заказа во Владивосток применяются тарифы CDEK/Почты России для внутренней доставки — эта сумма оплачивается отдельно по факту.\n\n"
