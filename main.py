...
async def calc_price_final(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"📦  <b>Дополнительно</b>: в стоимость также входят <b>страховка и проверка товара</b>. После прибытия заказа во Владивосток применяются тарифы CDEK/Почты России для внутренней доставки — <b>эта сумма оплачивается отдельно</b>🫶🏼\n\n"
