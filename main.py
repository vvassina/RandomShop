...
async def handle_category(message: types.Message):
    ...
    try:
        ...
        media.attach_photo(types.InputFile("price_example_1.jpg"))
        media.attach_photo(types.InputFile("price_example_2.jpg"))
        media.attach_photo(types.InputFile("price_example_3.jpg"))
        await bot.send_media_group(message.chat.id, media)
    except Exception as e:
        logging.error(f"Error sending images: {e}")
...
async def calculate_total(message: types.Message):
    ...
    try:
        ...
        current_category = None  # <--- СБРОС после расчёта!
    except Exception as e:
